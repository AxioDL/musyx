...
void* salPCAcquireSong(const void* input) {
  ...
  void* data;
  if (!salPCDecodeArrangement((SND_PC_SPAN){input, SIZE_MAX}, &data, NULL, NULL)) return NULL;
  PCSong* song = salMalloc(sizeof(*song));
  if (!song) { salFree(data); return NULL; }
...
void salPCCollectSongs(void) {
  ...
  while (*link) {
    PCSong* song = *link;
    if (song->references) { link = &song->next; continue; }
    *link = song->next;
    salFree(song->data);
    salFree(song);
  }
