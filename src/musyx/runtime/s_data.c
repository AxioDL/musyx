...
static bool pcPrepareRecord(PCRegistration* record, u16 id, u8 type, void* pool,
                            u32 newCount[5]) {
  MEM_DATA* data = NULL;
  void* existing = NULL;
  record->type = type;
  record->count = 0;
  switch (type) {
  case 0: data = GetMacroAddr(id, pool); existing = dataGetMacro(id); break;
  case 1: break;
  case 2: id |= 0x4000; data = GetKeymapAddr(id, pool); existing = dataGetKeymap(id); break;
  case 3: id |= 0x8000; data = GetLayerAddr(id, pool); existing = dataGetLayer(id, &record->count); break;
  case 4: data = GetCurveAddr(id, pool); existing = dataGetCurve(id); break;
  }
  record->id = id;
...
static void pcRemoveRecord(const PCRegistration* record) {
  switch (record->type) {
  case 0: dataRemoveMacro(record->id); break;
  case 1: dataRemoveSampleReference(record->id); break;
  case 2: dataRemoveKeymap(record->id); break;
  case 3: dataRemoveLayer(record->id); break;
  case 4: dataRemoveCurve(record->id); break;
  }
}

bool sndPushGroup(void* prj_data, u16 gid, void* samples, void* sdir, void* pool) {
  if (!sndActive || SP_CURRENT == 128 || !prj_data || !sdir) return false;
  SND_PC_GROUP_ASSETS raw = {{prj_data, SIZE_MAX}, {pool, SIZE_MAX},
                             {sdir, SIZE_MAX}, {samples, SIZE_MAX}};
  PCGroup prepared = {0};
  if (!salPCDecodeGroup(&raw, &prepared.assets, NULL)) return false;
  ...
  for (u32 i = 0; i < 5; ++i) {
    ...
    while (*list != 0xffff) {
      ...
      if (first & 0x8000) { first &= 0x3fff; last = *++list; }
      ++list;
      for (u32 id = first; id <= last; ++id) {
        if (!pcPrepareRecord(&prepared.records[index++], id, types[i], prepared.assets.pool, newCount))
          goto unlockFailed;
      }
    }
  }
  if (!salPCRegistrationSpace(newCount[0], newCount[4], newCount[2], newCount[3]))
    goto unlockFailed;
  if (!dataInsertSDir(prepared.assets.directory, samples)) goto unlockFailed;
  size_t inserted = 0;
  for (; inserted < prepared.count; ++inserted) {
    ...
    case 1:
      if (!dataAddSampleReference(record->id)) {
        while (inserted) pcRemoveRecord(&prepared.records[--inserted]);
        dataRemoveSDir(prepared.assets.directory);
        goto unlockFailed;
      }
      break;
    case 0: dataInsertMacro(record->id, record->payload); break;
    case 2: dataInsertKeymap(record->id, record->payload); break;
    case 3: dataInsertLayer(record->id, record->payload, record->count); break;
    case 4: dataInsertCurve(record->id, record->payload, record->size); break;
    }
  }
...
unlockFailed:
  hwEnableIrq();
failed:
  if (prepared.records) salFree(prepared.records);
  salPCFreeGroupData(&prepared.assets);
  return false;
}

bool sndPopGroup(void) {
  if (!sndActive) return false;
  hwDisableIrq();
  if (!SP_CURRENT) { hwEnableIrq(); return false; }
  ...
  for (size_t i = owned->count; i; --i) pcRemoveRecord(&owned->records[i - 1]);
  dataRemoveSDir(owned->assets.directory);
  if (group->type == 1) RemoveFXTab(group->id);
