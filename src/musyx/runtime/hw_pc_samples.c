...
void salPCFinishSampleUpload(void) {
  if (uploading) uploading->base = NULL;
  uploading = NULL;
}
...
static SDIR_DATA* findSample(SDIR_TAB* table, u16 id) {
  ...
  while (lo < hi) {
    ...
    if (table->data[mid].id < id) lo = mid + 1;
    else hi = mid;
  }
...
bool dataInsertSDir(SDIR_DATA* sdir, void* samples) {
  if (!sdir || directoryCount == 128) return false;
  for (u16 i = 0; i < directoryCount; ++i) {
...
bool dataRemoveSDir(SDIR_DATA* sdir) {
  for (u16 i = 0; i < directoryCount; ++i) {
    if (directories[i].data != sdir) continue;
    for (u16 j = 0; j < directories[i].numSmp; ++j)
      if (sdir[j].ref_cnt) return false;
    salPCFinishSampleUpload();
    memmove(&directories[i], &directories[i + 1],
            (--directoryCount - i) * sizeof(directories[0]));
    return true;
  }
  return false;
}
...
bool dataAddSampleReference(u16 id) {
  ...
  if (entry) {
    if (entry->ref_cnt == UINT16_MAX) return false;
    ++entry->ref_cnt;
    return true;
  }
  if (!uploading || !(entry = findSample(uploading, id))) return false;
...
s32 dataGetSample(u16 id, SAMPLE_INFO* sample) {
  ...
  if (entry->extraData) sample->extraData = (u8*)owner->data + entry->extraData;
  return 0;
}
